...
bathroom_count %>%
  ...
  labs (x = "Neighborhood",
        y = "Total Colony Count",
        title = 
          "Count of bacterial colonies in Outer Richmond and Civic Center")
```
...
bathroom_count_barplot <- ggplot(data = bathroom_count_table,
  ...
  geom_bar(stat='identity') +
    scale_fill_manual(values=c("#CDCDC1",
                               ...
                               "#CDB38B",
                               "#9FB6CD")) +
    labs(title = "Bacterial Morphotype description and its specific count",
...
ggmap(sf_basemap) +
  geom_point(data = bathroom_sample,
             aes(x = long_dec_degrees,
                 ...
                 shape = bathroom_type),
             size = 3) +
  labs(title = "Site sampling locations in San Francisco",
       x = "Longitude",
       y = "Latitude")
```
 ...
 bathroom_count %>%
    ...
    labs(title = "Total Colony Count On Bathroom Based On
      The Mean Median Income of San Francisco Neighborhoods",
       ...
       y = "Total Colony Count")
```
**Figure 6**: The total bacterial colony count was significantly different throughout the varied income (p < 0.05). 
...
bathroom_count %>%
  ggplot(aes(y = mean_median_population, # plot poulation
